...
class BlogPost(db.Model):
    ...
    author_id = db.Column(db.Integer, db.ForeignKey("users_new.id"))
...
class User(UserMixin, db.Model):
    ...
    # The "author" refers to the author property in the BlogPost class.
    comment = relationship("CommentPost", back_populates="comment_author")
...
def get_all_posts():
    ...
    if current_user.is_active:
        return render_template("index_bkp.html", all_posts=posts, login=1)
    else:
        return render_template("index_bkp.html", all_posts=posts)
...
def show_post(post_id):
    ...
    comments = CommentPost.query.all()
    form = CommentForm()
    if current_user.is_active:
        if request.method == "POST":
            new_comment = CommentPost(author_id=current_user.id, text=form.comment.data)
            db.session.add(new_comment)
            db.session.commit()
            return redirect(url_for('get_all_posts'))
        return render_template("post.html", post=requested_post, user_id=current_user.id, form=form, all_comments=comments)
    else:
        if request.method == "POST":
            flash(u'Please login first', 'error')
